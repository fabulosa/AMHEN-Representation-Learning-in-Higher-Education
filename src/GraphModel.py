...
class GATNEModel(nn.Module):
    def __init__(self, num_courses, num_students, embedding_size, embedding_u_size, edge_type_count, dim_a):
        ...
        #self.d = Variable(torch.Tensor(3,4)).cuda()
        # base embeddings for courses
    ...
    def forward(self, input_type, train_inputs, train_types, node_neigh):
        ...
        if input_type == 'c':  # course as input, self-attention
            ...
            course_embed = course_embed + torch.matmul(course_type_embed, trans_w).squeeze(1)
