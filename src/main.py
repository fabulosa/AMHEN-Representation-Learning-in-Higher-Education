...
def train_model(train_bi):
    ...
    model = GATNEModel(num_course, num_stu, embedding_size, embedding_u_size, edge_type_count, dim_a)
    nsloss = NSLoss(num_nodes, vocab_freq, num_sampled, embedding_size)
    #nsloss = CrossETPLoss(num_nodes, embedding_size)
    ...
    for epoch in range(epochs):
        ...
        for i in batch_shuffle:
            if i < len(train_pairs_course_loader):  # get batch with course as input
                ...
                optimizer.zero_grad()
                embs = model('c', x.tolist(), t.tolist(), neigh).cuda()
                # negative sampling
        ...
        for (x, y) in data_loader:
            ...
            course_embs = model('c', course, type, neigh).cuda()
        ...
        course_embed = F.normalize(model.course_embeddings, dim=1)
        course_embed = course_embed.cpu().data.numpy()
        ...
        model_embeddings = {'course_embed': course_embed, 'final_course_embed': final_course_embeddings,
    ...
    stu_embed = F.normalize(model.student_embeddings, dim=1)
    stu_embed = stu_embed.cpu().data.numpy()
    model_embeddings = {'course_embed': course_embed, 'final_course_embed': final_course_embeddings, 'stu_embed': stu_embed, 'final_stu_embed': final_stu_embeddings}
